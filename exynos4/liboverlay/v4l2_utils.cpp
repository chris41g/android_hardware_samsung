                                    ...
                                    , int32_t w, int32_t h, int rotation)
{
    ...
    LOGV("v4l2_overlay_set_position:: (%d,%d,%d,%d)=>(%d,%d,%d,%d) rot=%d"
                 , format.fmt.win.w.left, format.fmt.win.w.top, format.fmt.win.w.width, format.fmt.win.w.height
				 , rot_x, rot_y, rot_w, rot_h, rotation);
...
int v4l2_overlay_get_phy_Y_buf(int fd, int index, void **start, size_t *len)
{
    ...
    ctrl.id = V4L2_CID_GET_PHY_SRC_YADDR;
    ctrl.value = index;
    ret = v4l2_overlay_ioctl(fd, VIDIOC_S_CTRL, &ctrl, "get phy Y buf");
    *start = (void*)ctrl.value;
...
int v4l2_overlay_get_phy_C_buf(int fd, int index, void **start, size_t *len)
{
    ...
    ctrl.id = V4L2_CID_GET_PHY_SRC_CADDR;
    ctrl.value = index;
    ret = v4l2_overlay_ioctl(fd, VIDIOC_S_CTRL, &ctrl, "get phy C buf");
    *start = (void*)ctrl.value;
