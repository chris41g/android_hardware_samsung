        ...
        overlay_handle_t handle)
{
    ...
    ctx->phyYbuffers  = new void* [ctx->num_buffers];
    ctx->phyCbuffers  = new void* [ctx->num_buffers];
    ctx->buffers_len = new size_t[ctx->num_buffers];

    if (!ctx->buffers || !ctx->phyYbuffers ||!ctx->phyCbuffers ||
            !ctx->buffers_len || !ctx->mapping_data) {
        ...
        if (ctx->zerocopy)
            ...
            for (i = 0; i < ctx->num_buffers; i++) {
                ...
                rc = v4l2_overlay_get_phy_Y_buf(ctx->ctl_fd, i,
                        &ctx->phyYbuffers[i], &ctx->buffers_len[i]);
                if (rc) {
                    LOGE("Failed mapping buffers\n");
                    goto error;
                }
                rc = v4l2_overlay_get_phy_C_buf(ctx->ctl_fd, i,
                        &ctx->phyCbuffers[i], &ctx->buffers_len[i]);
                if (rc) {
                    LOGE("Failed mapping buffers\n");
                    goto error;
                }
...
error:
    ...
    if(ctx->buffers)
        delete [] ctx->buffers;
    if(ctx->phyYbuffers)
        delete [] ctx->phyYbuffers;
    if(ctx->phyCbuffers)
        delete [] ctx->phyCbuffers;
    if(ctx->buffers_len)
    ...
    if (!ctx->zerocopy) {
        for (int i = 0; i < ctx->num_buffers; i++) {
            v4l2_overlay_map_buf(ctx->ctl_fd, i, &ctx->buffers[i],
                    &ctx->buffers_len[i]);
            v4l2_overlay_get_phy_Y_buf(ctx->ctl_fd, i,
                        &ctx->phyYbuffers[i], &ctx->buffers_len[i]);
            v4l2_overlay_get_phy_C_buf(ctx->ctl_fd, i,
                        &ctx->phyCbuffers[i], &ctx->buffers_len[i]);
        }
    }
...
int overlay_queueBuffer(struct overlay_data_device_t *dev,
    ...
    struct ADDRS * ptrAddr = (struct ADDRS *) buffer;

    unsigned int color_space = 0;

    android::HdmiService * hdmiService = android::HdmiService::getInstance();

    set_color_space(ctx->format, &color_space);

    hdmiService->postVideo2HDMI(ctx->width,
                           ctx->height,
                           color_space, //ctx->format, //HAL_PIXEL_FORMAT_CUSTOM_YCbCr_420_SP,
                           /*android::SecHdmiService::HDMI_MODE_VIDEO,*/
                           (unsigned int)ptrAddr->addr_y,
                           (unsigned int)ptrAddr->addr_cbcr,
                           0,0);

#endif // BOARD_USES_HDMI
        ...
        overlay_buffer_t buffer)
{
    ...
    switch ((int)buffer & 0xF0) {
    case 0x80:
        if (((int)buffer & 0xF) >= 0 && ((int)buffer & 0xF) < ctx->num_buffers)
            return (void*) ctx->phyYbuffers[((int)buffer & 0xF)];
        break;
    case 0x40:
        if (((int)buffer & 0xF) >= 0 && ((int)buffer & 0xF) < ctx->num_buffers)
            return (void*) ctx->phyCbuffers[((int)buffer & 0xF)];
        break;
    case 0:
