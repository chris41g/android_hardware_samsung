...
void *SsbSipMfcEncOpenExt(void *value)
{
	...
	ioctl(hMFCOpen, IOCTL_MFC_SET_BUF_CACHE, &CommonArg);
	if (CommonArg.ret_code != MFC_OK) {
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcEncGetInBuf(void *openHandle, SSBSIP_MFC_ENC_INPUT_INFO *input_info)
{
	...
	pCTX->virFrmBuf.luma = pCTX->mapped_addr
		+ user_addr_arg.args.mem_alloc.offset;
	pCTX->virFrmBuf.chroma = pCTX->mapped_addr
		+ user_addr_arg.args.mem_alloc.offset
		+ (unsigned int)aligned_y_size;
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcEncGetOutBuf(void *openHandle, SSBSIP_MFC_ENC_OUTPUT_INFO *output_info)
{
	...
	output_info->StrmVirAddr = (void *)(pCTX->virStrmBuf + pCTX->mapped_addr);
